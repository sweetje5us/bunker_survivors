...
export class AboutScene extends Phaser.Scene {
  ...
  private createTitle(): void {
    ...
    // Основной заголовок "ABOUT"
    const mainTitle = this.add.text(0, 0, 'ABOUT', {
  ...
  private createContent(): void {
    ...
    const contentContainer = this.add.container(centerX, contentY)
    
    // Фон для контента
    const contentBg = this.add.rectangle(0, 0, 600, 200, 0x8b0000, 0.2)
    contentBg.setStrokeStyle(2, 0xff0000)
    
    // Текст контента
    const contentText = this.add.text(0, 0, 'BUNKER PROTOCOL\n\nPrototype UI\nCredits: You\n\nВерсия: 1.0.0\n\nСистема безопасности\nактивирована', {
    ...
    contentContainer.add([contentBg, contentText])
